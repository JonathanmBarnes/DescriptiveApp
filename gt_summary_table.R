...
# This function is partially altered from the original in gtExtra to fit needs
# of this application. Please see original here: https://jthomasmock.github.io/gtExtras/reference/gt_plt_summary.html
...
gt_plt_summary <- function(df, title = NULL) {
  ...
  tab_out <- sum_table %>%
    ...
    fmt_percent(n_missing, decimals = 2) %>%
...

  {
    if (utils::packageVersion("gt")$minor >= 6) {
      tab_out %>% sub_missing(Mean:`3rd Quartile`)
    } else {
      tab_out %>% fmt_missing(Mean:`3rd Quartile`, missing_text = "--")
    }
  }
}
...
create_sum_table <- function(df) {
  sum_table <- df %>%
    ...
    dplyr::mutate(
      ...
      `1st Quartile` = ifelse(type %in% c("double", "integer", "numeric"), quantile(value, probs = 0.25, na.rm = TRUE), NA),
      `3rd Quartile` = ifelse(type %in% c("double", "integer", "numeric"), quantile(value, probs = 0.75, na.rm = TRUE), NA)
    ) %>%
    dplyr::ungroup() %>%
...
plot_data <- function(col, col_name, n_missing, ...) {
  ...
  if (col_type %in% c("factor", "character", "ordered factor")) {
    ...
    plot_out <- ggplot(df_in, aes(x = x)) +
      ...
      {
        if (length(unique(col)) > 2) geom_vline(xintercept = mean(col, na.rm = TRUE), linetype = "dotted")
      } +
      theme_void() +
      theme(
        ...
        text = element_text(family = "Trebuchet", size = 6)
      )
